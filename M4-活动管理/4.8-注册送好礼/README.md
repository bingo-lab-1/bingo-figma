...
| 时区 | 注册、领取受理与到账时间按平台业务时区分别记录,展示注明时区 |
...
| 彩金打码倍数(A5) | 必填,正数、最多两位小数;缺失不得当作零倍。注册取得资格时与金额同时锁定,手动领取成功入账时才建立独立任务,见D72 |
...
平台注册成功时记录资格;玩家查看奖励并主动点击领取时才受理发奖。

输入信息：可信注册事实、登录玩家与运营商、已有资格及领取请求;金额、币种和倍数由服务端读取资格快照,不由玩家提交。

1. 按A2记录唯一资格。注册及资格处理不会调用资金入账,资格处理失败不撤销已经成功的注册;恢复只补回按原注册时点应有的资格。
2. 领取前展示可领金额、币种、倍数、领取后新增流水及平台投注模式,明确提示“领取后不能自行取消;该币种仍有未完成打码任务时,包含后续充值在内的全部余额均按资金规则限制提现”。预计流水不作为已生效任务展示。
3. 玩家主动领取时,服务端核对登录身份、运营商、资格归属、账号资金限制及原领取结果。无论平台采用哪种投注模式,都不以是否充值作为领取前提。已领取返回原结果,处理中查询原请求,不得创建另一笔奖。
4. 同一资格的并发点击与不同请求编号也只能受理同一份奖励。领取成功必须同时完成彩金入账、建立绑定的独立打码任务和记录领取结果;不得出现余额已增加但任务缺失,也不得先建立无对应到账奖励的生效任务。
...
| 4.8-R4 | 玩家查看说明并主动领取 | 不要求先充值;彩金和绑定任务同时成功,任务按资格快照金额乘倍数建立 |
| 4.8-R4 | 同一注册消息重复、并发领取或资金超时重试 | 同一玩家同一运营商只有一份资格、一笔奖励及一条对应任务;未知结果恢复原请求 |
| 4.8-R4 | 已领取后请求取消、奖励输完或长期未完成任务 | 不提供玩家取消,服务端拒绝取消请求;任务不因输完或时间经过而清除 |
| 4.8-R4 | 原币种停用或账号资金受限时点击领取 | 拒绝新入账并说明原因,保留资格或原请求,不创建无到账奖励的生效任务 |
...
| 注册资格与手动领取(4.8-R4) | 注册成功由系统记录资格;领取必须由登录玩家本人主动请求,服务端校验归属及去重 |
| 展示与执行平台投注限制(4.8-R7) | 本页查看用 view;投注为系统判定 |
| 查询活动记录(4.8-R8) | `activity:register-gift:view` |

> 注册资格与手动领取统一在R4描述;R5编号保留,不另设重复的领取流程。R6关联充值翻倍本期不提供。
...
- 拥有(owner=M4):活动配置与版本、注册领取资格及快照、领取受理依据、实际发放结果及时间;不保存翻倍订单或奖池结果。
