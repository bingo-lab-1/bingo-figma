---
名称: 注册送好礼
路由: /gameCenter/activity/register-gift
模块: M4
优先级: P1
状态: 设计中
更新: 2026-09-07
...
> 本轮业务确认:注册赠送一笔彩金,运营配置打码倍数;充值翻倍延后。
> 平台基础设置拥有“充值投注限制 / 无限制投注”,本活动不另设同名开关。
> 来源 Excel 的旧翻倍方案已不属于本期;已定与剩余假设见 [M4 决策清单](../README.md#7-决策清单) D66–D73。

## 1. 背景与目标

**目标**:新玩家注册后获得一笔彩金,清楚看到金额及流水要求;能否在充值前使用彩金游戏,由平台统一投注模式决定。
...
**业务价值**:注册即获得明确奖励;平台可选择先充值后游戏,也可允许玩家先用赠金体验。
...
- 不做虚拟奖金、谢谢参与、转盘抽奖;固定奖励金额是本稿简化假设,见 D67。
...
| 编号 | 参数 | 取值 | 依据 | 在哪配 |
|---|---|---|---|---|
| A1 | 配置范围 | 每运营商一套活动;名称暂按 1–40 字符;保存草稿后发布生效 | 假设,D69 | 本页 |
| A2 | 触发与次数 | 注册成功赠送一笔;本稿暂按服务端自动发放,每运营商每玩家一次,重开或换版本不重置,不补活动开启前注册的老用户 | 注册赠金已定;自动发放与次数边界为假设,D66 | 服务端 |
| A3 | 奖励金额 | 单个正数,符合奖励币种精度;不设中奖概率或未中奖 | 固定金额为简化假设,D67 | 本页 |
| A4 | 奖励币种 | 活动级一个,平台已开通币种;变更须确认金额数值不自动换算 | 沿用活动奖励币种约定 | 本页 |
| A5 | 彩金打码倍数 | 必填;本稿暂按正数、最多两位小数;缺失不得当作零倍。发奖独立建立义务,倍数与金额同时锁定 | 可设置倍数已定;正数精度为假设,D72 | 本页 |
| A6 | 平台投注模式 | 充值投注限制 / 无限制投注,准确语义见资金模型“平台充值前投注资格”;平台设置实时决定后续投注资格,不绑定活动版本 | 已定,D73;动态切换为执行假设 | 平台基础设置,本页只读 |
| A7 | 已充值判定 | 暂按玩家在当前运营商内至少一笔成功到账充值,不要求从活动入口充值,不额外设金额门槛;充值发起或失败不算,赠金不算充值 | 操作口径假设,D73;人工充值、拒付等边界未定 | 财务提供事实,投注侧判定 |
| A8 | 生效与关闭 | 发布仅影响新受理赠金;关闭停止新增,已经受理的发奖继续按原快照处理。已入账彩金不因活动关闭或未充值作废 | 原彩金不作废原则;发奖受理边界为假设,D69 | 本页 |
| A9 | 发放恢复 | 结果未知保持处理中并查原结果;确认未入账后恢复原发奖动作,金额与依据不变。重复注册消息不重复发奖 | 幂等要求,恢复细节为假设,D72 | 服务端 |
...
| ID | 需求 | 触发条件 | 验收标准 | 权限码 |
|---|---|---|---|---|
| 4.8-R1 | 查看与保存草稿 | 进入、编辑后保存 | 草稿不改变生效配置;投注模式只读 | activity:register-gift:edit(查看用 view) |
| 4.8-R2 | 核对与发布 | 点击发布并确认 | 金额与倍数完整有效;旧发奖快照不变 | activity:register-gift:publish |
| 4.8-R3 | 开启与停用 | 切换开关 | 控制新增赠金,不追回已发奖 | activity:register-gift:switch |
| 4.8-R4 | 注册赠金自动发放 | 注册成功 | 不要求充值;单次入账及独立打码,重复消息不重发 | 系统触发 |
| 4.8-R7 | 展示与执行平台投注限制 | 查看活动、玩家提交投注、充值到账 | 两种模式只改变充值前投注资格,不改变发奖额或打码义务 | 本页查看用 view;投注为系统判定 |

> R5 普通手动领取与 R6 关联充值翻倍为旧稿编号,本期撤出,不复用。
> R4 自动发放是按“注册会送一笔奖励”采用的明确假设,见 D66。
...
**可输入数据**:第 5 段可编辑配置;平台投注模式只读。

**功能范围**:界面提供格式提示;服务端隔离运营商、判权限和草稿版本。

**处理逻辑**:查看用 activity:register-gift:view,保存用 activity:register-gift:edit。
允许不完整草稿,但已填写字段须合法;并发覆盖拒绝并保留输入。展示当前模式及其解释,不在活动草稿保存模式值。

**错误处理**:无权限拒绝;保存失败保留输入;基础设置取数失败显示“投注模式暂不可用”,不可默认显示无限制。
...
**功能范围**:界面显示实际彩金与需打码量;服务端校验发布与留痕。

**处理逻辑**:按 A1/A3–A5 校验名称、币种、金额和倍数,确认后形成发布版本;确认期间草稿改变则重新核对。
首次发布仍需开启;后续发布按 A8 生效。不把充值翻倍、标签、CPF 或活动互斥能力作为发布条件。

**错误处理**:缺倍数、非法金额或币种失效拒绝发布;不得以默认倍数补齐缺失配置。
...
**功能范围**:界面说明影响;服务端执行权限与开关留痕。

**处理逻辑**:开启须有可用发布版本;关闭按 A8 执行,不修改平台投注模式。
重新开启不重新奖励已经参与的玩家。

**错误处理**:无生效配置禁止开启;失败返回实际状态。
...
**可输入数据**:可信注册事实、玩家与运营商、当时生效配置;金额与倍数不由玩家提交。

**功能范围**:活动决定赠金;资金负责入账与打码,注册流程提供成功事实。
...
**错误处理**:资金超时保持处理中;缺倍数或币种不可用保留待核查并告警;不显示虚假已到账。
...
**触发条件**:查看奖励说明、充值成功到账、每次提交投注或平台模式改变。

**可输入数据**:当前平台模式、可信充值事实;钱包余额、封禁和游戏状态按原有规则校验。

**功能范围**:平台基础设置拥有模式;投注服务端统一判定,活动只展示和发奖。
...
**错误处理**:未成功充值不能用“已发起充值”放行;配置未知不可默认无限制;重复充值通知不触发本活动发奖。
...
- 4.9 将消费玩家、版本、金额币种、倍数、需打码量、发放时间与处理结果,暂不展开页面。
