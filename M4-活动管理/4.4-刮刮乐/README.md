...
这里定义后台配置及玩家取得、使用卡片的业务规则,不规定刮奖动画、页面布局或组件样式。记录查询另由4.5承载。活动不单独做跨账号IP/设备去重或渠道字段;参与人群使用用户标签。异常充值、退款、封禁处置不在本次修订范围内。
...
新增卡片配置的校验范围待D85收敛后补齐;本次整理不代填默认值。
...
完成一次刮奖,无论中奖与否都消耗一次当日次数,同一张卡不能重复使用。未中奖则显示未中奖;中奖则按发卡时确定的金额和币种自动发放彩金,同时产生打码要求,无需后续充值。
...
达到持卡上限后,不能再发卡使数量超过上限。之后空出位置,是否补发当时因满额没有拿到的卡,修订没有说明,见D85。
...
| 4.4-R3 | 关闭活动后发生新充值 | 不再新发卡;已入账彩金和打码不收回,旧卡是否可刮待D85 |
...
| 4.4-R4 | 持卡只剩一个名额时两笔合格充值同时处理 | 不能超上限发卡;满额充值之后是否补发待D85 |
| 4.4-R4 | 每日只剩一次时同时刮两张卡 | 最多完成一次刮奖,中奖与否都只扣该次 |
| 4.4-R4 | 同一充值重复通知、同一卡重复刮 | 不重复发卡、计次或发奖 |
| 4.4-R4 | 刮开中奖卡 | 按发卡时奖金与倍数结算,无需再次充值;改配置不重抽 |
| 4.4-R4 | 卡片已按其确定期限过期 | 不可刮,不占持卡上限;具体期限算法待D85 |
| 4.4-R4 | 没有匹配档位或标准计价缺失 | 前者正常不发;后者记录核查且不阻塞充值,补发政策待D85 |
| 4.4-R4 | 中奖资金结果未知 | 保留原结果供核查,不重抽、不重复发钱;恢复和计次细则待D85 |
...
| 操作 | 权限码 |
|---|---|
| 查看配置 | `activity:scratch-card:view` |
...
**记录查询**:4.5刮刮乐活动记录仍需补齐卡片机制的查询规格。至少应查清用户、原充值、匹配档位、发卡时间、有效期、卡片状态、锁定的奖金币种金额与倍数、刮奖时间、当日次数、未中奖或中奖结果及关联资金记录。失败和结果未知也要可追溯,不能只保留成功入账记录。
