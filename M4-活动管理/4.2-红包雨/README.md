...
| A6 | 金额档位 | 按 VIP 等级分档,每档填等级区间和金额区间;等级区间两两不重叠(端点也不能碰);最小金额 ≤ 最大金额;至少一档最大金额 > 0;**所有档位合起来要盖住当前存在且启用的全部等级**,有缺口保存时拦 | 前三条已验证(线上文案);重叠由服务端拦已定(D43);覆盖口径 2026-09-01 随 D58 修订 —— 等级可删且序号不复用,序列会不连续(如 0 1 2 4 5),原「盖住 0 到最高等级」的连续区间口径作废,**档位区间可以跨过空洞,空洞不算缺口** | 本页 |
...
| VIP 档位有缺口 | VIP X ~ Y 等级没有配置金额,请补齐(只列**存在且启用**的等级,已删除的号不提示) |
...
- VIP 等级(**归 M4,模块内**):领取那一刻的等级决定金额档。等级序列由 M4 的 VIP 页拥有,
  本页只读它的等级清单来配金额档 —— 等级增删会直接影响 A6 的覆盖校验,见 E7。
...
| E12 | 场内等级被删或停用 | 一场进行中运营在 4.6 删掉或停用了某个等级 | 停在该等级的玩家按 A13 处理(不发放 + 告警);等级删除的连锁处理见 M4-D60,拍板后本行随之收敛。**已领的不追回**(与 E5 同口径) |
