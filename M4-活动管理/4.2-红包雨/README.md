...
当天充过值的玩家到点在线就能点红包拿一笔小钱。这一页就管红包雨的所有规则:时段、金额、加成、门槛、文案、开关。
...
| 玩家 | 当天充一笔后,到点在线点红包领钱(在玩家端) | 没有理由在固定时间回来,活跃拉不起来 |
| 财务 / 风控 | 看每天发了多少、有没有被小号薅 | 发出去多少钱说不清,被薅了也查不到 |

**业务价值**:把玩家在固定时间拉回来,在线时长、留存和当日充值靠这个撑;发的钱要玩够量才能提,
...
| 参与说明 | 在时段内但领不了(次数用完、今日没充值等)时弹出的说明文案 |
...
| A2 | 时段 | 每场填开始和结束时刻(精确到秒);开始 < 结束;各场按时间先后排、两两不重叠(端点也不能碰);最后一场不跨天,最晚 23:59:59 | 已验证(线上文案);不重叠由服务端保存时拦已定(D43),线上界面未拦 | 本页 |
...
| A6 | 金额档位 | 按 VIP 等级分档,每档填等级区间和金额区间;等级区间两两不重叠(端点也不能碰);最小金额 ≤ 最大金额;至少一档最大金额 > 0;**所有档位合起来要盖住当前存在且启用的全部等级**,有缺口保存时拦 | 前三条已验证(线上文案);重叠由服务端拦已定(D43);覆盖口径随 D58 修订 —— 等级可归档且序号不复用,序列会不连续(如 0 1 2 4 5),原「盖住 0 到最高等级」的连续区间口径作废,**档位区间可以跨过空洞,空洞不算缺口** | 本页 |
...
| A13 | VIP 不在任何档位 | **不发放**,按"不可领取"处理,并给运营告警提示补档;不按最低档发 | 已定(2026-08-28 业务确认,D43),配合 A6 覆盖校验 | 写死 |
| A14 | 改配置什么时候生效 | 活动开关和纯文案立刻生效;时段、次数、金额、加成、打码倍数、标签**下一场才生效**,正在进行的一场按开场时的规则走 | 已定(2026-08-28 业务确认,D43) | 写死 |
| A15 | 加成按标签分组 | 加成表可以按用户标签分成多组,每组一张表;按优先级取第一个命中的组,都没命中用默认表 | 已定,本期做(2026-08-28 业务确认,D44) | 本页 |
...
| A19 | 参与门槛:当日充值 | 玩家点领取那一刻,今日(运营时区)已有**至少一笔成功到账的充值**,金额不限;没有则不可领,弹参与说明;充值发起但未到账不算 | 已定(2026-08-31 业务确认,见 `../README.md` 第 7 段 D51) | 写死 |
...
| 4.2-R4 | 到点下雨与领取算钱 | 定时到场次开始 / 玩家点红包 | 当日充过值的玩家每场限 1 次(A19);按 A6 ~ A13 算出金额;彩金入账并记打码;写一条领取记录 | 无(服务端自动) |
...
3. 查金额档:等级区间两两不碰;每档最小 ≤ 最大;至少一档 > 0;合起来盖住当前存在且启用的全部等级(A6)。
...
| VIP 档位有缺口 | VIP X ~ Y 等级没有配置金额,请补齐(只列**存在且启用**的等级,已归档的号不提示) |
...
flowchart TD
    ...
    C -- 是 --> C2{今日已有充值到账?}
    C2 -- 否 --> X5[弹参与说明<br/>提示充值后可领]
    C2 -- 是 --> D{这一场领过了?<br/>或今日次数用完?}
    D -- 是 --> X3[弹参与说明]
    D -- 否 --> E{VIP 落在某档?}
    E -- 否 --> X4[不发放<br/>给运营告警]
    E -- 是 --> F[在该档区间随机出基础金额]
...
2. 资格:活动开着、命中活动标签、在时段内、**今日已有至少一笔充值到账**、本场没领过、今日次数没用完(A3、A4、A16、A19)。
...
- 正向:今日充了 5 元,充值和亏损都没到任何加成门槛 → 可以领,系数 0,领到基础金额本身。
- 逆向:同一人同一场连点三次 → 只成功一次,只加一次钱。
- 逆向:时段外点红包 → 拒绝并弹未开启提示,不加钱不记账。
- 逆向:今日没充值,时段内点红包 → 拒绝并弹参与说明,不加钱;充值到账后同场再点,可领(A19)。
...
- VIP 等级(**归 M4,模块内**):领取那一刻的等级决定金额档。等级序列由 M4 的 VIP 页拥有,
  本页只读它的等级清单来配金额档 —— 等级新增、停用与归档会直接影响 A6 的覆盖校验,见 E7。
...
| E6 | 场内等级变动 | 玩家在一场内从 VIP 5 升到 6,或因保级流水未达成降到 VIP 4 | 以点领取那一刻的等级为准,**升降级同一口径**;已领的不回溯、不补差也不追回 |
| E7 | 高 VIP 无档位 | 系统新增了更高 VIP,档位没跟上 | 不发放,告警提示补档(A13);保存时 A6 覆盖校验兜底 |
| E12 | 场内等级被停用 | 一场进行中运营在 4.6 停用了某个等级 | 停在该等级的玩家按 A13 处理(本场不发放 + 告警),已领的不追回。按 M4-D60 当前推荐方案,4.6 不允许归档仍有玩家的等级;停用等级的玩家迁往已启用的更低等级后,按目标等级参加后续场次 |
...
| E11 | 先点后充 | 玩家时段内点红包被拒(今日未充值),随后充值到账并在同一场再点 | 以再次点击那一刻判断,已有到账充值即可领(A19、A12);发起未到账不算,0 点后跨日充值不算前一日 |
...
- 只读:钱包和账变、打码任务、充值汇总(归财务)· 投注与派彩(归玩法,本仓库范围外)· 用户标签命中集(归 M2)· VIP 等级序列(**归 M4,模块内**)
