...
| 条件行 | 是 | 1 到 A1 行 |
| 行 · 条件类型 | 是 | 基础指标 / 属性(D56) |
| 行 · 左值 | 是 | 按条件类型从基础指标或属性清单下拉选,不可自由输入;同一项允许出现多次 |
| 行 · 左值时间范围 | 指标时是 | 只列该指标的可用范围(A8);选最近N天须填 N(A7)。属性恒为当前时点,不显示时间范围 |
| 行 · 比较符 | 是 | 数值与有序属性用 `= ≠ > ≥ < ≤`;渠道来源用「属于 / 不属于」 |
| 行 · 右值 | 是 | 指标条件选常量或白名单内基础指标,按 A7、A12 校验;VIP等级单选当前等级;渠道来源从当前启用渠道中多选,同一条件内多个值按 OR |
...
| 金额类的币种口径(A9) | 一律按**标准计价币种 USD** 判定;每笔账变按入账时刻汇率换算,累计与切片值从账变上的换算记录汇总,不新增汇率计算逻辑。换算尚未补算的账变**不计入**,该玩家该指标视为 **NULL**,按 R6 判否；标准币写死(S1)；(D24),见 `../2.1-用户列表/标准计价.md` |
| 每页条数(A10) | 50 条/页,与 2.1 一致 |
| 试算响应上限(A11) | 10 秒,超时中断并提示,不返回半截结果；系统配置 |
| 右值与可比白名单(A12) | 右值为常量,或**左值白名单内**的「基础指标 + 时间范围」;左右时间范围可不同,唯一排除是同指标同时间范围。白名单语义、名单初值与允许/禁止示例见 `条件模型.md` 第 5 段,白名单严格使用条件模型第 5 段列举项,未列入的组合拒绝保存；名单为系统配置；(D31 修订) |
...
以及 VIP等级、渠道来源两项当前属性。
账户余额不开放(D35);登录次数按去重后的登录成功事件计数。清单可扩展,
...
2. 校验每行:所选基础指标或属性存在于当前清单且可用。指标条件的时间范围须合法(A8),
   最近N天的 N 符合 A7;属性条件不得提交时间范围。
3. 校验右值:指标常量按 A7 校验类型与精度;右值为基础指标时按 A12 校验白名单、
   时间范围合法且不得与左值同指标同时间范围。VIP等级只能选存在的等级;
   渠道来源只能选当前渠道字典中的值,使用「属于 / 不属于」且至少选择一个渠道;
   同一渠道条件内多选天然为 OR,不受标签级 AND / OR 影响。
...
2. 按组合方式逐行判定:**AND 全部满足才命中,OR 任一满足即命中**;净盈亏按带号值比较。
   渠道条件内部的多个选值先按 OR 算成一行真假,再参与标签级 AND / OR
...
> **规则模型**(基础指标、属性、时间范围、可比白名单、0 与 NULL 的判定语义)见同目录
...
> 模型词(基础指标、属性、时间范围、条件、比较符、可比白名单)的定义与示例见
...
- **拥有**(owner=M2):标签 · 标签的条件组成 · 命中关系 · 进出流水 · 基础指标与属性清单 ·
  可比白名单与各指标可用时间范围配置 · R9 所需标签规则版本与只读评估能力
...
|---|---|
| 权限判定与可见范围过滤 | **服务端** |
| 名称唯一、行数、指标 / 属性有效性、时间范围、比较符、白名单、值域与精度校验 | **服务端** |
| 命中判定、差集比对、打标摘标与流水写入 | **服务端** |
| 重算调度、中断续跑、熔断与告警 | **服务端** |
| 删除前的引用精确判定与事务内复校 | **服务端** |
| 试算的只读保证与超时保护 | **服务端** |
| R9 规则版本留存、业务时点评估与依据完整性、未知恢复及只读保证 | **服务端** |
| 条件行的增删、条件类型 / 时间范围 / 比较符 / 右值联动、组成预览、互斥即时提示 | 界面 |

### 新增编辑标签的职责(2.5-R2)

| 归属 | 做什么 |
|---|---|
| 界面 | 条件行的增删、类型 / 时间范围 / 比较符 / 右值联动、组成实时预览、离开前提示未保存 |
| 服务端 | 名称唯一、行数、条件项有效性、时间范围、比较符、白名单、值域与精度校验;判权限;保存后登记待重算 |
