...
**目标**:让运营用平台预置的**基础指标与属性**组合出**条件**,再由条件组合出**标签**,
新增一类人群当天生效,不用等发版。
...
| ID | 需求 | 触发条件 | 验收标准 | 权限码 |
|---|---|---|---|---|
| 2.5-R1 | 查看标签列表 | 进入页面 · 点击查询 | 能按名称、状态、条件项筛出标签,展示组成与命中人数 | `user:tag:view` |
...
**可输入数据**:第 5 段表单字段。基础指标与属性只能从清单选;时间范围、比较符与右值按所选条件项联动。
...
- **只读**:资金汇总与切片统计(owner=M2,见 `../README.md` 第 4 段)· 玩家档案字段(owner=M2,定义在 2.1)·
  登录与设备记录(owner **未验证**,见第 8 段)· 渠道字典与玩家注册渠道(owner=代理渠道,**本仓库范围外**)·
