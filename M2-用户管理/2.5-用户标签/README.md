...
| A7 | 值的精度 | 金额类两位小数且带币种;次数、天数、小时数为非负整数 | 假设 | 写死 |
| A8 | 首版基础标签范围 | 只含累计类,不含当日、昨日等时间切片 | 已定 | 基础标签清单 |
| A9 | 金额类的币种口径 | 现有汇总**不区分币种**,首版按单一汇总值判定 | **未验证假设**,多币种下语义失真,见 `../README.md` D24 | 待定 |
...
|---|---|---|
| 标签名称 | | 是 |
| 组成 | 全部基础标签拼成一行,如「累计充值 ≥ 50 且 累计充值次数 < 2」;超长折叠 | 是 |
...
4. 每次打标与摘标都写一条进出流水,记录标签、玩家、方向、时间、原因,
   以及**判定当时的值** —— 没有它,事后无法解释「他当时为什么被打上」
...
| E8 | 基础标签下线 | 某基础标签不再可用但仍被标签引用 | 引用它的标签自动停用并告警,不得静默按不命中跑 |
