...
| 用户标签 | tag | 标签为自定义的标签;**≥ 3 个时折叠**:只显示前 2 个 + 「+N」,悬停「+N」浮层展示全部,见下方「用户标签折叠」 |
...
| 充值总额 | float | 用户累计成功充值,**换算为标准币 USD 展示**;列头带说明悬浮(见下方「列头说明组件」),换算规则见 [`标准计价.md`](./标准计价.md) |
| 提现总额 | float | 用户累计成功提现,同上按标准币 USD 展示,列头带说明悬浮 |
| 充提差总额 | float | 充值总额 − 提现总额,标准币 USD,列头带说明悬浮 |
...
- **列头说明组件**:充值总额 / 提现总额 / 充提差总额的列头带「?」图标,悬停显示口径说明——按标准币 USD 换算、每笔账变按入账时刻汇率、只读派生视图不作任何资金判定依据(见 [`标准计价.md`](./标准计价.md))。该悬浮说明抽成**通用组件**,供其他需要口径解释的列复用(截图中用户ID、邀请人等列头已有 ? 图标,即此形态)
