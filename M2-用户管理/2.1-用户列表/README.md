...
| 5 | VIP等级 | select | 是 | 默认「不限」;下拉读 M4 的等级序列,展示已启用的全部等级(含已停用等级上仍有玩家的情形) |
...
“时间”下拉只提供注册时间和首充时间;最后投注、最后存款、最后登录使用各自独立日期框,各条件之间按AND组合。
...
| 配置或参数 | 填写与执行规则 |
|---|---|
| 未指定时间时的默认查询窗(A1) | **不限时间**,按注册时间降序取首页；每页条数见 A2;靠排序与分页兜住数据量,不靠时间窗 |
| 每页条数(A2) | 50 条/页；实采为 50 条/页(`截图/列表-表格与分页.png`) |
| 进入页面是否自动查询(A3) | **否**,点「查询」才取数；减轻服务端压力 |
| 查询响应上限(A4) | 5 秒；超过即中断并给可读提示,不返回半截数据。这是**约束不是目标**:实测达不到就改查询实现,不是把这个数调大 |
| 在线状态判定(A5) | 首选按 WebSocket 连接状态:断开超过 **60 秒**宽限期判离线;客户端不支持长连接时退化为**近 5 分钟内有请求/心跳**；客户端每60秒上报心跳;两种判定并存时以长连接为准 |
| 导出行数上限(A6) | 5 万行；取决于导出服务的承载能力,需实测 |
| 导出链接有效期(A7) | 24 小时；防止文件在对象存储里无限累积 |
...
| 2 | 会员账号 | text | 注册时所用账号(手机号 / 邮箱等);通过 Google 注册的展示 **Google 账号的邮箱**——此时与「邮箱」列取值重复,属预期。按账号精确搜索时,Google 注册玩家即以该邮箱命中 |
| 3 | 邮箱 | text | 绑定邮箱,未绑定为空;脱敏见 E6 |
| 4 | 真实姓名 | text | 实名认证姓名,未实名为空;脱敏见 E6 |
| 5 | 手机号 | text | **账号绑定手机**(不是提现绑定手机,两者是不同字段;提现绑定手机仍只作筛选项);数字,最长 20;脱敏见 E6 |
| 6 | 上级代理 | text | 玩家归属的代理,**与「邀请人」不是一回事**(见“相关资料”术语);点击打开该代理的详情弹窗,**不跳转**代理管理页 |
| 7 | 推广渠道 | text | 玩家注册来源的渠道 |
| 8 | 会员标签 | tag | 自定义标签;**≥ 3 个时折叠**:只显示前 2 个 + 「+N」,悬停浮层展示全部,见下方「用户标签折叠」 |
| 9 | 账号状态 | enum | 仅 **正常 / 封禁** 两态,无中间状态;封禁口径与封禁列表 2.4 一致 |
| 10 | 在线状态 | enum | 在线 / 离线,判定见 A5 |
| 11 | VIP等级 | number | 0-N,单一等级序列(M2-D1);**只读引用 M4**,本页不写入 |
...
点击「查询」按钮 · 筛选框内回车 · 打开别人发来的带筛选条件的链接

输入信息： 见查询条件及结果字段两节

查询条件校验、分页取数、脱敏及权限过滤按相关资料中的数据流执行。

### 用户详情弹窗(2.1-R4)

点表格「用户ID」· 点「操作」列的「详情」· 打开别人发来的该玩家详情链接

输入信息： 一个玩家。玩家不存在或已注销时按 E4 处理
...
### 16 项组合筛选遇到异常时(2.1-R2)

参数非法 → 定位到具体字段的提示

无结果 → 空态"未找到匹配用户",并提示可放宽的条件

超时 → "查询范围过大,请收窄时间或增加条件",**不返回半截数据**

### 用户详情弹窗遇到异常时(2.1-R4)

玩家不存在/已注销 → 弹窗内提示,不出现系统错误页(E4)

无权限 → 不显示入口;绕过界面直接取数同样被拒绝并提示无权限(E3)

**某个 tab 取数失败 → 只有该 tab 显示重试,其余 tab 照常可用**

弹窗打开期间该玩家被他人封禁 → 提交操作时提示状态已变更(E5)
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 2.1-R1 | 用户列表与分页：进入页面 / 翻页 | 默认按注册时间倒序;每页条数见 A2;**进入页面不自动查询**(A3);分页与总数准确 |
| 2.1-R2 | 16 项组合筛选：查询 / 回车 / 打开分享链接 | 16 项任一及组合生效;收起态只露前 6 项;重置清空;**当前筛选可分享给同事** |
| 2.1-R3 | 列设置：勾选列 | 勾选即时生效并按账号持久化 |
| 2.1-R4 | 用户详情弹窗：点用户ID / 点「详情」/ 打开分享链接 | 弹窗展示 10 个 tab,切到哪个才加载哪个;关闭后回到原筛选与滚动位置 |
| 2.1-R5 | 导出：点导出 | **不阻塞页面**,完成后站内信通知;下载链接有效期见 A7 |
| 2.1-R6 | 多币种单钱包与统一余额：某币种首次产生资金往来 | 同玩家同币种只有一个钱包与一个余额;冻结与提现占用不计入可用;余额不为负 |
| 2.1-R7 | 资金来源账本与账变流水：任何一次余额变化 | 无流水不得改余额;流水记变动前/金额/变动后与来源;冲正写反向流水不改原记录 |
| 2.1-R8 | 充值及彩金创建独立打码任务：充值到账 / 彩金入账 / 人工调整 | 每笔独立建任务不合并;充值固定 1 倍,彩金按活动倍数;人工调整可填 0 倍但需豁免权限、原因与留痕;**倍数缺失一律拒绝入账,不退化为 0 倍** |
| 2.1-R9 | 有效投注与 FIFO 跨任务核销：一笔投注最终结算成功 | 严格 FIFO;单笔可连续冲抵多条任务且总量守恒;超额部分不形成打码余额;重复推送只核销一次 |
| 2.1-R10 | 按币种钱包执行提现拦截：玩家发起提现 / 后台审核 | 该币种存在任意 active 任务即整个币种禁提;币种之间互不牵连;被拦时告知还差多少 |
| 2.1-R11 | 跨币种闪兑(**延后**)：玩家确认报价并提交闪兑 | 整笔原子成败,不出现单边账;报价过期或币种停用即拒绝;同一请求重复提交只成交一笔 |
| 2.1-R12 | 闪兑的打码义务迁移(**延后**)：一次闪兑成功执行 | 逐条任务按可兑换余额比例拆分;目标任务继承来源与任务链;往返闪兑义务不增不减 |
| 2.1-R13 | 币种、汇率、报价与精度配置：运营修改币种或汇率配置 | 精度、汇率、报价有效期均可配且留痕;缺配置时该币种不可用而非套默认值 |
| 2.1-R14 | 玩家端钱包与闪兑展示：玩家打开钱包 / 发起闪兑 | 玩家只见单一余额与"还差多少";闪兑确认页展示将迁移的打码量;展示舍入不写回 |
| 2.1-R15 | 自动对账与不平告警：按周期自动执行 / 手动发起 | 钱包、流水、闪兑、任务、派彩两轨每期核对;不平即告警并冻结该钱包提现与闪兑 |
| 2.1-R16 | 扣款顺序与返奖回冲：下注扣款 / 该注派彩 / 提现扣款 | 下注彩金优先扣光为止、提现真金优先,两个方向相反;打码按下注全额递减与出资来源无关;出资比例下注瞬间冻结,派彩按该比例回冲且两轨之和恒等于派彩额 |
| 2.1-R17 | 人工打码干预(增加/清除)：运营给某币种加打码量 / 清除某条任务 | 加码填绝对值不走倍数、允许超过余额;**清除只能逐条,不提供一键清空**;两者均需原因与留痕并进对账关注项 |

### 16 项组合筛选(2.1-R2)

**正向**:单条件 / 三条件组合 / 打开带筛选的分享链接 → 结果与条件一致,总数准确

**逆向**:最小值大于最大值 → 被拒绝并指出是哪个字段;不填时间 → 按 A1 取首页,且出结果快于 A4;查询超时 → 给出可读提示而不是白屏或半截数据


### 用户详情弹窗(2.1-R4)

**正向**:点用户ID 打开 → 停在「用户详情」→ 切到「资产变动」时该 tab 才开始加载 → 关闭后列表的筛选条件与滚动位置都没变

**逆向**:分享链接里的玩家已注销 → 弹窗内提示,不是错误页;无权限账号绕过界面取数 → 被拒绝;某个 tab 加载失败 → 只有它显示重试,其余 tab 仍能正常查看


### 导出(2.1-R5)

**正向**:1 万行导出 → 收到通知,文件行数与列表总数一致,敏感字段已脱敏

**逆向**:超 A6 行 → 被拒绝并告知当前行数;无权限的人绕过界面发起导出 → 被拒绝;超 A7 后再点下载链接 → 已失效;结果里含测试账号 → 文件中不出现
...
| 操作 | 权限或执行方 |
|---|---|
| 用户列表与分页(2.1-R1) | `user:list:view` |
| 16 项组合筛选(2.1-R2) | `user:list:view` |
| 列设置(2.1-R3) | `user:list:view` |
| 用户详情弹窗(2.1-R4) | `user:detail:view` |
| 导出(2.1-R5) | `user:export` |
| 多币种单钱包与统一余额(2.1-R6) | `finance:wallet:view` |
| 资金来源账本与账变流水(2.1-R7) | `finance:ledger:view` |
| 充值及彩金创建独立打码任务(2.1-R8) | `finance:wager:adjust` |
| 有效投注与 FIFO 跨任务核销(2.1-R9) | `finance:wager:view` |
| 按币种钱包执行提现拦截(2.1-R10) | `finance:withdraw:review` |
| 跨币种闪兑(**延后**)(2.1-R11) | `finance:fx:view` |
| 闪兑的打码义务迁移(**延后**)(2.1-R12) | `finance:fx:view` |
| 币种、汇率、报价与精度配置(2.1-R13) | `system:currency:config` |
| 玩家端钱包与闪兑展示(2.1-R14) | — |
| 自动对账与不平告警(2.1-R15) | `finance:reconcile:view` |
| 扣款顺序与返奖回冲(2.1-R16) | `finance:ledger:view` |
| 人工打码干预(增加/清除)(2.1-R17) | `finance:wager:add` `finance:wager:clear` |

> 本段是**索引**:扫一眼知道这一页要做哪些事、各自需要什么权限。
> 详细规格在“操作规则”,两段不重复表述。
>
> **R1–R5 是页面行为**,规格在“操作规则”;**R6–R17 是这一页所有金额的底层资金规则**,
...
> **统一定义在同目录 [`资金模型.md`](./资金模型.md) 第 2 段术语表**,此处不重复。
...
界面负责:收集条件、即时提示格式错误、把当前筛选体现在链接里

**服务端负责:条件合法性、结果正确性、超时保护。**

界面上的校验只为改善体验,**不能作为唯一防线**

### 用户详情弹窗的职责(2.1-R4)

界面负责:弹窗开合、tab 切换、把当前看的是谁体现在链接里(以便分享)

**服务端负责:每次取数都要判权限、每次返回都要脱敏。**

**界面上"看不见"不算数据安全**——不允许把完整数据发给界面再靠隐藏遮住(见 E3、E6)

### 导出的职责(2.1-R5)

界面负责:发起导出、展示进度

**服务端负责:排队、取数、脱敏、生成文件、完成通知、到期失效。**

导出**不得阻塞页面**,运营发起后可以继续做别的事
