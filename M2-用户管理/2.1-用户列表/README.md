...
| A5 | 在线状态判定窗口 | 近 5 分钟内有活动 | 假设 | 取决于玩家端多久上报一次,需确认 |
...
| 字段 | 类型 | 可输入数据 / 校验 |
|---|---|---|
| 用户ID | text or number | ID 为数字;**精确匹配**,需考虑查询速度最快，点击展开用户弹窗 |
| 邀请人 | text or number | 邀请人用户ID，点击展开用户弹窗 |
| 用户昵称 | text | 最长 32 字符;**精确匹配** |
| 用户标签 | tag | 标签为自定义的标签 |
| 用户账号 | tag | 理论上为用户注册时候用的账号，例如手机号，邮箱号，通过google注册的话需要考虑展示哪个为用户账号 |
| 在线状态 | select | 全部 / 在线 / 离线(判定见 A5) |
...
| 总流水金额 | float | 用户总共下注的流水额度,**带币种标识** |
| 总充值金额 | float | 用户总共成功充值的额度,**带币种标识** |
| 总提现金额 | float | 用户总共成功提现的额度,**带币种标识** |
| 余额 | float | 该币种的**单一余额**;多币种玩家按币种分别呈现。不含冻结与提现占用 |
| 真金 | float | **账本派生的来源构成**(L3),不是独立余额 —— 见 `资金模型.md` R7 |
| 彩金 | float | 同上,来源为活动发放的部分 |
...
- **用户标签列很宽**(单个玩家常挂十几个标签,截图里一行能挂满 15 个),需要想办法处理
...
| 11 | 充值金额最小值 | number | 否 | 筛出累计充值 **大于等于** 该值的玩家 |
| 12 | 充值金额最大值 | number | 否 | 筛出累计充值 **小于等于** 该值的玩家;小于最小值时拒绝并指出字段 |
...
| 7 | 资产变动 | 所有余额变动流水;总额是余额,真金彩金是**来源构成**,打码量是该币种**全部任务剩余量合计** |
...
> 详细规格在第 7 段,两段不重复表述。
>
> **R1–R5 是页面行为**,规格在第 7 段;**R6–R15 是这一页所有金额的底层资金规则**,
...
| 2.1-R8 | 充值及彩金创建独立打码任务 | 充值到账 / 彩金入账 / 人工调整 | 每笔独立建任务不合并;充值固定 1 倍,彩金按活动倍数;人工调整需二次确认与原因 | `finance:wager:view` |
...
| 2.1-R11 | 跨币种闪兑 | 玩家确认报价并提交闪兑 | 整笔原子成败,不出现单边账;报价过期或币种停用即拒绝;同一请求重复提交只成交一笔 | `finance:fx:view` |
| 2.1-R12 | 闪兑的打码义务迁移 | 一次闪兑成功执行 | 逐条任务按可兑换余额比例拆分;目标任务继承来源与到期时间;往返闪兑义务不增不减 | `finance:fx:view` |
...
| 2.1-R15 | 自动对账与不平告警 | 按周期自动执行 / 手动发起 | 钱包、流水、闪兑、任务四方每期核对;不平即告警并冻结该钱包提现与闪兑 | `finance:reconcile:view` |
