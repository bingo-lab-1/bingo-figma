...
> 结构基于**自身代码**(`bingo-backend` 现有数据模型与接口)+ 竞品参考(`参考资料/竞品-aigobet`)。
> 思维导图表达**结构**;跨模块的**依赖关系**看下方第二张流程图——两张配套才完整。
...
| 缺口 | 现状 | 后果 |
|---|---|---|
| **打码进度** | 钱包 有 `balance` / `bonusBalance`,但**无 wagering/turnover 字段** | 彩金无法挂流水要求 → 送出去的彩金可直接提走,薅羊毛入口 |
...
3. **身份体系 SSOT**——实名认证 / 用户标签 / 身份信息 三套并存,谁是主?
