...
- 不做活动 ROI 报表(归 M1,本模块只产生 活动领取记录 原始数据)
...
| 活动 | 一次可配置的奖励规则 | 活动 |
| 活动编码 | 活动的业务唯一标识 | 活动编码 |
| 领取记录 | 用户从活动获得奖励的流水 | 活动领取记录 |
| 资格圈人 | 谁能参与(渠道/标签/VIP) | `eligibility` |
| 奖励结构 | 档位/矩阵/日历/里程碑四态 | `rewardKind` |
| 厂商 | 第三方游戏提供商 | 游戏厂商 |
| 公平性 | provably fair 种子验证 | 公平性记录 |

## 3. 依赖关系

**上游依赖(我引用)**
- `M2-用户管理`:用户 用户标签 VIP等级(资格圈人)
- `M5-代理渠道`:渠道 代理(按渠道/代理差异化)
- `M3-财务管理`:发彩金写 账变记录、挂 打码进度

**下游被依赖(谁引用我)**
- `M3-财务管理`:打码进度的来源标识 以**元数据 ID** 引用活动(非外键)
- `M1-数据中心`:活动领取记录 投注记录
- `M6-运营管理`:Banner/弹窗引用 活动标识
...
| 表 | 说明 | 现状 |
|---|---|---|
| 活动 | 活动主表 | 需改造 · **待重构为配置化引擎** |
| 活动领取记录 | 统一领取记录 | 新增 |
| ~~9 张各活动独立配置~~ | 一个活动一张表 | 大缺口 · **待合并进引擎** |
| 游戏厂商 / 游戏类型 / 游戏 | 游戏与厂商 | 已有 |
| 游戏热度 | 热门位 | 已有 |
| 公平性记录 / 公平性历史 | 公平性校验与历史查询 | 已有 |
| 投注记录 | 每笔投注的下注与结算 | 已有 |
| 大厅布局 | 首页分类与推荐位编排 | 新增 |
...
**验收**:用引擎表达 10 个活动(现有 6 个 活动编码 + 竞品实测 4 个),全部能表达才通过。
...
| 4.7 | 大厅编排 | `/game/lobby` | P1 | 需改造 · 仅 游戏热度 | 5 |
