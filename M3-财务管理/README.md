...
**做什么**:钱的进出与审核。钱包 / 账变记录 / 各类订单的唯一写入方。
...
**上游依赖(我引用)**
- `M2-用户管理`:用户(订单归属)、用户标签(提现风控)
- `M5-代理渠道`:渠道(渠道归因)
- `M8-系统设置`:加密币种 打码倍数配置
- `00-公共约定`:审核状态 币种类型

**下游被依赖(谁引用我)** ← 改表前先看这行
- `M1-数据中心`:账变记录 及订单表(充提报表)
- `M2-用户管理`:钱包 账变记录(用户详情展示)
- `M4-玩法中心`:发彩金 → 写 账变记录、挂 打码进度
- `M5-代理渠道`:佣金结算 → 写 账变记录
...
| 表 | 说明 | 现状 |
|---|---|---|
| 钱包 | 真金、彩金、冻结分账余额 | 需改造 · 需加冻结余额 |
| 账变记录 | 账变总账 | 已有 |
| 充值订单 | 从下单到到账的全过程 | 新增 |
| 提现订单 | 从申请到打款的全过程 | 新增 |
| 提现审核规则 | 自动过审与转人工的匹配规则 | 新增 |
| 提现配置 | 提现全局配置 | 新增 |
| 人工上下分记录 | 人工上下分 | 新增 |
| 打码进度 | 每笔充值或彩金的流水解锁进度 | 新增 |
| 冻结记录  | 冻结/解冻流水 | 新增 |
| 充值商品 | 充值商品 SKU | 新增 |
| 支付通道 | 支付/提现通道 | 需改造 · 仅加密 |

**只读引用**:用户 用户标签(M2)· 渠道(M5)· 打码倍数配置(M8)
### 两个必补字段(全库 grep 命中 0)
| 缺口 | 后果 |
|---|---|
| 钱包的冻结余额 | 提现拒绝后冻不住钱,多路径解冻无处落地 |
| 打码进度 | 有倍数配置但无进度追踪 → **充值与彩金都能不打码直接提走** |
