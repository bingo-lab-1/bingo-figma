...
玩家邀请直属下级获得充值与投注返佣,平台以可控成本换取自增长拉新;
...
拉新成本与下级真实充值投注挂钩;每笔佣金带完整计算依据,可追溯。
...
- 本页不设计玩家端页面;直属邀请与佣金交互见[Account需求](../../玩家端/Account/README.md#account-r13--邀请与佣金),计算与领取规则仍由本页维护
...
| 配置或参数 | 填写与执行规则 |
|---|---|
| 活动模式(A1) | 枚举:仅直属 / 直属+二级 / 直属+团队 / 三级分销;**首版只开放仅直属**,其余置灰,数据结构预留；活动配置 |
| 充值返佣比例(A2) | 1% ~ 10%,精度 0.1%;范围写死,值由运营配；活动配置 |
| 投注返佣档位(A3) | 1 ~ 10 档;阈值严格递增、比例非递减;默认见上表；活动配置 |
| 佣金打码倍数(A7) | 0 ~ 10 倍,0 = 无要求,默认 3 倍;**领取入账时**按记录快照的倍数产生打码任务,语义见 `资金模型.md`；活动配置；默认值来自现行后台截图 |
| 人群限定口径(A10) | 用户标签多选取**并集**,开放渠道多选取**并集**,两维度之间取**交集**;「全部」= 不限(含无标签玩家);**限定对象是代理本人**,不限定其下级；活动配置 |
...
两类佣金均按**自然日**(运营时区)汇总生成待领取记录,次日到达「领取开放时点」后可领取;开放时点后台可配,默认 00:00。

配置或来源：活动配置。

### 判档口径(A5)

判档按全部直属下级的**终身累计**有效投注,取最高满足档;发放基数为**当日新增**有效投注,按结算时点档位全量单一比例,**非分段累进**。

配置或来源：终身判档;增量发放为唯一不重复返佣的读法,随 D25 一并视为。

### 充值返佣发放时机(A6)

充值到账时逐笔计算生成待领取记录,**不实时入账**,次日按 A4 开放领取。

### 金额口径(A8)

判档与计算一律按标准计价币种 USD;换算记录尚未补算的充值/注单**不计入当期**,补算后计入下期,对齐 `标准计价.md`。

### 佣金入账币种(A9)

以标准计价币种 USD 入账至代理钱包。

配置或来源：前提是钱包开设 USD,实现时不支持须回 D26 重议。

### 资格判定时点(A11)

每次计算按**当时**标签命中与渠道归属判定;不符合即不发,恢复后**不补发**。

### 有效投注折算(A12)

有效投注 = 实际投注 × 游戏折算比例;按游戏逐个配置,owner=游戏管理,**本仓库范围外,只读引用**,未配置按 100% 处理;比例修改只对新注单生效,不追溯。

配置或来源：游戏管理。

### 领取机制(A14)

佣金一律由代理**手动领取**(R7),领取时入账并按 A7 产生打码任务;待领取佣金**不过期**。
...
充值订单成功到账(owner=财务,只读)。

输入信息：订单金额与换算结果 · 该玩家的上级(代理关系,只读)· 活动配置快照 ·
上级的标签命中与渠道归属。
...
结算周期(A4)结束,系统调度,无手工触发界面。

输入信息：有直属下级的玩家 · 下级当日已结算注单的有效投注(A12/A8)·
周期结束时点的配置快照。
...
代理在玩家端对待领取佣金发起领取,业务支持单条或一键全部;[Account本期界面](../../玩家端/Account/README.md#account-r13--邀请与佣金)只提供单条记录领取,规则以本段为准。

输入信息：待领取佣金记录(含比例与打码倍数快照)· 领取开放时点(A4)。
...
> 待领取不过期(A14)。佣金记录不含「打码中 / 可提现」状态 —— 打码进度归打码任务(`资金模型.md`),不重复承载。
...
测试账号、自我邀请、风控标记账号的充值与投注不计入;刷佣判定规则归风控(**范围外**)。
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 7.1-R1 | 查看活动配置：进入页面 | 展示全部配置、启停状态与最近修改人 |
| 7.1-R2 | 编辑活动配置：修改并保存 | 按 A1~A3、A7、A10 校验;只对之后的计算生效,**不追溯**;留痕 |
| 7.1-R3 | 启停活动：切换并二次确认 | 关闭即停止新佣金;历史记录保留可查 |
| 7.1-R4 | 充值返佣计算：直属下级充值到账 | 按 A6 逐笔计算生成**待领取**记录;同一订单幂等只生成一次 |
| 7.1-R5 | 投注返佣日结：结算周期结束 | 按 A5 终身累计判档、当日增量计算,生成**待领取**记录;批次可中断续跑,重复调度幂等 |
| 7.1-R6 | 佣金冲回：来源充值冲正/退款 | 未领取的记录直接**作废**;已领取的自动冲回**以钱包余额为上限**(不产生负余额),未耗打码任务核销;差额只在冲回留痕中记录并告警,**首版不做挂账台账与系统内追偿**,线下人工处理(D27 已定,有意省略勿补)。投注返佣结算后的注单回滚**不追溯**,超阈值告警 |
| 7.1-R7 | 佣金领取入账：代理在玩家端发起领取 | 校验状态与开放时点(A4/A14);入账带来源标识 + 按 A7 产生打码任务;重复领取幂等 |
...
| 操作 | 权限或执行方 |
|---|---|
| 查看活动配置(7.1-R1) | `agent:commission:view` |
| 编辑活动配置(7.1-R2) | `agent:commission:edit` |
| 启停活动(7.1-R3) | `agent:commission:toggle` |
| 充值返佣计算(7.1-R4) | 系统触发,无界面权限 |
| 投注返佣日结(7.1-R5) | 系统触发,无界面权限 |
| 佣金冲回(7.1-R6) | 系统触发,无界面权限 |
| 佣金领取入账(7.1-R7) | 系统触发,无界面权限 |
