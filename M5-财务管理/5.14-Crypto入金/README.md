...
支持玩家通过多条区块链、多个币种向自己的专属地址入金,达到确认条件后自动到账,
同一笔链上资金只入账一次且全程可追溯。
...
| 客服 | 按用户、地址或交易哈希确认充值进度 | 玩家说没到账时无法判断是链上确认中、充错链还是系统漏单 |
| 财务 | 核对链上实收、平台入账与异常记录 | 链上资产与玩家余额无法闭环对账 |
| 风控 | 识别异常来源、重复交易和高风险入金 | 可疑资产可能在发现前已被提现或消费 |

扩大可接受的支付方式,减少共享地址和人工认领带来的错账,
并把链上事实、充值订单与玩家余额连成可核对的一条链。

### 本次范围

- 不向玩家或后台运营展示助记词、根密钥、私钥或派生密钥
- 不在本页执行链上提现、归集、换币或法币结算；归集方案独立见
  `Gas-Station与自动归集方案.md`
...
| 充值打码倍数 | 首次登记入金时锁定的运营商级 X,只读;处理轨迹可查配置版本及关联打码任务 | 否 |
| 状态 | 当前链上与入账状态 | 是 |
...
| 配置或参数 | 填写与执行规则 |
|---|---|
| 入账确认数(A1) | 按网络配置,各网络独立；Crypto网络配置；链最终性不同,不能全平台写死 |
| 最小入金金额(A2) | 按网络 + 币种配置;低于阈值只记录不自动入账；Crypto币种配置 |
| 入金手续费(A3) | MVP 为 0,链上实收多少就入账多少；Crypto币种配置 |
| 地址分配规则(A4) | 每个玩家每条网络一个稳定专属地址;协议要求 Memo/Tag 时地址与 Memo/Tag 共同标识玩家；地址规则配置；用户已明确要求专属钱包 |
| 同链多币种地址(A5) | 网络协议允许时共用该玩家在该网络的地址,币种仍按资产标识独立入账；避免为每个代币重复派生地址 |
| 监听起点(A6) | 网络启用时记录的起始区块,不得扫描起点之前的历史交易；Crypto网络配置 |
| 到账时效目标(A7) | 达到 A1 后 2 分钟内完成入账或进入明确异常态；监控告警配置 |
| 地址生成时机(A8) | 玩家首次打开该网络充值页时按需分配,重复打开返回原地址 |

**充值打码统一来源**:X 只在 [5.5 充值商品与赠送](../5.5-充值商品配置/README.md)顶部“充值通用配置”按运营商维护,默认初始化为 1、必须为正数。Crypto 入金与商品充值共用该值,本页及 5.15 不另设倍数。无商品下单的入金在首次持久化记录时锁定 X 及版本;配置修改不改变确认中记录、重新核验或补单的原依据。X 缺失或非正数须转异常、停止入账,不得默认取 0 或 1。统一充值 X 不表示 Crypto 自动加入商品赠送方案。
...
已登录玩家首次打开某个已启用网络的 Crypto 充值页;或已有玩家再次取地址。

输入信息：当前玩家身份 · 网络;玩家不得提交用户ID、派生序号或自定义地址。
...
玩家进入 Crypto 充值页或切换币种、网络。

输入信息：币种 · 网络;只能选择后台已启用且绑定正确链上资产标识的组合。
...
受支持网络观察到资产转入平台已分配的用户专属地址。

输入信息：网络 · 区块高度与哈希 · 交易哈希 · 事件/输出序号 · 收款地址 ·
资产唯一标识 · 原始链上金额 · 发送地址 · 链上时间。

1. 只处理 A6 起点之后、收款地址属于平台有效地址池的转入事件
2. 通过网络 + 资产唯一标识识别币种,不得只看名称或符号
3. 以“网络 + 交易哈希 + 事件/输出序号 + 资产唯一标识”作为业务唯一事实;
   相同事件无论被轮询、推送或重扫多少次都只登记一次;首次持久化时同时锁定运营商的充值打码倍数 X 及配置版本,重扫不得覆盖
...
8. 无法取得有效 X 时仍保存可审计链上事实,记录配置异常并暂停入账;不得以最新值静默修补已登记记录的历史依据

### 确认后自动入账(5.14-R4)

已登记事件达到 A1,且资产、地址、金额和风控状态全部允许入账。

输入信息：入金记录 · 最新主链确认结果 · A1/A2/A3/A9 的处理快照 · 首次登记时锁定的充值 X 及配置版本。
...
3. 校验 Currency 为 Crypto 类型、状态正常且允许充值;检查 A2、A9 与入金幂等状态;
   校验入金记录锁定的 X 存在且为正数;任何条件不满足都不得进入余额写入
4. 按链上实收减 A3 计算入账金额,费用为 0 时两者相等;不在此流程换汇
5. 生成来源为 Crypto 的充值订单,保存网络、链上资产标识、Currency ID、currencyCode、
   交易哈希、事件/输出序号、发送与收款地址、区块和确认阈值快照,继承首次登记时锁定的 X 及版本
6. 按用户ID + currencyCode 定位唯一平台钱包;不存在时在本次原子操作内创建,
   已存在时使用原钱包
7. 入该钱包的真金轨,按资金模型以**实际到账充值金额 × 锁定 X**创建独立打码任务并写账变流水;不得按当前最新配置重算
...
持有重新核验权限的运营对未入账记录点击重新核验,填写原因并二次确认。

输入信息：入金记录ID · 原因(必填);不得手工填写金额、币种、地址、哈希或确认数。

1. 重新核验不是人工改状态,所有链上字段必须重新读取并验证
2. 满足 R4 时允许自动入账;不满足时保持对应状态并返回明确原因
3. 已入账记录只允许核对,不得再次入账
4. 操作人、原因、前后状态与核验结果必须留痕
5. 始终沿用原入金记录的 X;重新核验不是修改倍数或重新获取奖励机会的入口
...
| E16 | 充值倍数变更 | 首次登记后 X 从 2 改为 3 | 原入金记录、后续订单与重新核验仍按 2,修改后新登记记录按 3,既有任务不重算 |
| E17 | 充值倍数缺失或非法 | X 未初始化、无法解析、为 0 或负数 | 保留链上事实并转配置异常,不得入账或默认用 0/1;人工调整豁免不适用于普通 Crypto 入金 |

### 分配用户专属地址遇到异常时(5.14-R1)
...
| 充值倍数快照缺失或非法 | 保留链上事实并转配置异常,本次不入账;提示打码依据不完整,不静默使用当前倍数 |

### 重新核验异常记录遇到异常时(5.14-R6)
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 5.14-R1 | 分配用户专属地址：玩家首次打开某网络充值页 | 返回可归属该用户的稳定地址;并发请求不生成两个地址 |
| 5.14-R2 | 多链多币种展示：玩家选择币种与网络 | 只展示已启用组合及对应地址、Memo/Tag、最小额和确认要求 |
| 5.14-R3 | 发现并登记链上入金：监听到专属地址收到资产 | 每个链上事件只登记一次,金额、币种、用户和链上凭据可追溯 |
| 5.14-R4 | 确认后自动入账：确认数达到 A1 且通过校验与风控 | 生成一笔充值订单并原子完成余额、流水和打码任务写入 |
| 5.14-R5 | 查询入金与处理轨迹：进入页面 · 查询 · 翻页 | 可按用户、地址、哈希、网络、币种和状态查到完整处理链 |
| 5.14-R6 | 重新核验异常记录：对未入账记录点击重新核验并填写原因 | 重新读取链上事实但不修改事实;满足条件时仍只入账一次 |
| 5.14-R7 | 导出入金记录：点击导出 | 导出与当前筛选一致,敏感字段按权限脱敏且不含任何密钥材料 |
...
- 正向:首次登记时 X=2,确认期间改为 3 → 原记录仍保留 2 与原版本,新事件锁定 3
...
- 正向:实收并到账 100 USDT、首次登记锁定 X=2 → 单独建立 200 USDT 打码任务;修改通用配置后重新核验或补单不改变该任务
- 逆向:资产映射的 Currency 已关闭充值 → 只登记入金并转配置异常,不得转入默认币种
- 逆向:达到 A1 前重复处理多次 → 始终不入账;达到后并发处理 → 只入账一次
- 逆向:余额已写但打码任务写入失败 → 整笔回滚,余额与订单均不留半成品
- 逆向:首次登记时 X 缺失、为 0 或负数 → 保留链上事实且明确异常,不得按默认 1 倍或免打码入账
...
| 操作 | 权限或执行方 |
|---|---|
| 分配用户专属地址(5.14-R1) | `finance:recharge:crypto` |
| 多链多币种展示(5.14-R2) | `finance:recharge:crypto` |
| 发现并登记链上入金(5.14-R3) | `finance:recharge:crypto` |
| 确认后自动入账(5.14-R4) | `finance:recharge:crypto` |
| 查询入金与处理轨迹(5.14-R5) | `finance:recharge:crypto` |
| 重新核验异常记录(5.14-R6) | `finance:recharge:crypto:recheck` |
| 导出入金记录(5.14-R7) | `finance:recharge:crypto:export` |

### 术语与共用口径

> 钱包、余额、真金、打码任务与账变流水的语义统一引用
> `../../M2-用户管理/2.1-用户列表/资金模型.md`;充值订单与补单引用 `../5.2-充值订单查询/README.md`。
> 充值通用 X 的唯一配置入口见 [5.5 充值商品与赠送](../5.5-充值商品配置/README.md);本页仅保存和执行快照。
> 链、币种、确认数、资产标识与 RPC 节点的配置入口统一见 `../5.15-Crypto配置/README.md`。
> 入账后的链上资产归集、补 Gas 与统一钱包方案见 `Gas-Station与自动归集方案.md`。
