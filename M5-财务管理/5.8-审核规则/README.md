...
用标签圈出的人群决定提现走人工还是自动放款,人工只看被拦下的单,每笔单的去向有据可查。
...
低风险人群的提现从「等人上班」变成分钟级到账;人工火力集中在可疑单;
...
| 配置或参数 | 填写与执行规则 |
|---|---|
| 总开关(A1) | 只决定**未命中任何标签**的订单去向:开=自动放款,关=转人工。**默认关**;切换均须二次确认(开:展示当前规则清单;关:提示自动提现标签仍生效) |
| 规则行唯一(A7) | 同一标签最多配置一行 |
...
命中集与标签规则求交集:含「转人工审核」即转人工(冲突时**人工优先**);全为「自动提现」则直接放款,**不受总开关影响**;交集为空按 A1。

### 永不拒绝(A3)

自动审核不拒绝订单,拒绝只能由人工在 5.1 做出。

### 生效时点(A5)

修改只对之后生成的订单生效,不追溯、不重判。

### 标签读取口径(A6)

读判定时刻的**当前命中集**;重算延迟窗口内的漏判是已知代价,不做提现时同步重算。
...
> R1~R3、R5 为简单查询与维护操作,“验收场景”一行即为完整规格,不展开。

### 自动审核判定(5.8-R4)

提现订单生成,即提现资格校验与金额占用完成之后。执行在提现下单流程,口径以本段为准。

输入信息：玩家当前标签命中集(owner=M2)· 标签规则与总开关的配置快照。

1. 取配置快照,读玩家当前命中集(A6);读取失败或故障按 A4 转人工并告警
2. 求命中集与标签规则的交集,按本节「判定去向」表分流(A1/A2)
...
命中集或配置读取失败、判定故障,一律**转人工**并留痕告警 —— 缺数放行等于错付钱。
...
| E5 | 引用的标签停用 | 已配置的标签在 2.5 被停用 | 该行永不命中 —— 整行标异常并告警,不得静默失效;删除已被 2.5 的引用预检阻断(2.5-A6) |
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 5.8-R1 | 查看审核规则：进入页面 | 展示总开关与全部标签规则,列构成见“配置项” |
| 5.8-R2 | 维护标签规则：新增、修改处置或删除 | 新增按 A7 校验;删除二次确认;按 A5 生效;留痕前后对照;版本冲突提示重载,不静默覆盖 |
| 5.8-R3 | 切换总开关：切换并二次确认 | 按 A1 生效并留痕;重复提交只生效一次 |
| 5.8-R4 | 自动审核判定：提现订单生成(执行在提现下单流程,口径以本页为准) | 按 A2~A4 判定;每单写判定快照;转人工的单带命中原因进 5.1 |
| 5.8-R5 | 判定快照查询：在 5.1 打开订单详情或审核弹窗 | 展示该单快照:走到的分支、命中标签与处置 |
...
| 操作 | 权限或执行方 |
|---|---|
| 查看审核规则(5.8-R1) | `finance:withdraw:rules` |
| 维护标签规则(5.8-R2) | `finance:withdraw:rules:edit` |
| 切换总开关(5.8-R3) | `finance:withdraw:rules:toggle` |
| 自动审核判定(5.8-R4) | 系统触发,无界面权限 |
| 判定快照查询(5.8-R5) | `finance:withdraw:order` |
