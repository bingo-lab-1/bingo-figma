...
让财务在一个页面里完成提现的审核与放款跟踪,并保证两个人不会同时审同一笔单。
...
提现是玩家对平台信任度最敏感的环节,慢一小时的体感损失远大于快一小时的收益。
同时它也是资金流出的唯一闸口,审错一笔就是真金白银的损失。这一页要同时做到**快**和**不出错**。

### 本次范围
- 不做提现规则的配置(通用限额与手续费由财务基础配置管理,审核阈值归审核规则,VIP 逐级每日次数 / 金额上限归 M4.6)
- 不做打码义务的判定逻辑本身(见“相关资料”引用的资金模型,本页只消费判定结果)
- 不做放款通道的对接配置(那是通道管理的事)
- 玩家端提现申请交互见[Account需求](../../玩家端/Account/README.md#account-r6--提现与收款账户),本页维护受理、报价、占用与审核的资金口径
...
| 用户ID | text | 数字,精确匹配 |
| 订单状态 | select | 默认待审核,见 A2。待审核 / 审核中 / 放款中 / 放款成功 / 放款失败 / 已拒绝 / 已取消 / 结果核查中 |
| 订单号 | text | 精确匹配 |
...
| 提现方式 | 玩家选择的收款形式 | 是 |
| 订单状态 | 当前状态 | 是 |
| 转人工原因 | 自动审核转人工的原因(命中的标签与处置,或未命中且未启用自动放行),来自 5.8 判定快照 | 是 |
| 操作人员 | 审核人;自动放款的单记为系统 | 是 |
| 备注 | 审核或拒绝原因 | 是 |
| 操作 | 审核 · 查看详情 | 是 |

**详情新增只读判定快照**:申请业务日期 · 申请时 VIP 等级 · 当日最高次数 / 金额 ·
本单标准币金额 · 申请前已用次数 / 金额 · 受理结果。该快照用于解释限额判定,不可在本页编辑。
...
| 每页条数(A1) | 20 条/页 |
| 默认筛选(A2) | 订单状态为待审核 |
| 自动刷新间隔(A3) | 30 秒；配置项,可由运营开关 |
| 批量审核单次上限(A4) | 50 笔；配置项 |
| 查询响应上限(A5) | 5 秒；写死,超时中断并给可读提示 |
| 导出行数上限(A6) | 5 万行；配置项 |
...
本期Account只接入支持USD法币且手续费从申请总额内扣的渠道。M5提供申请总额、钱包占用总额、手续费、预计到账及有效报价依据:钱包占用等于申请总额,预计到账等于申请总额减手续费,VIP金额额度仍按手续费前申请总额占用。申请总额须大于0,手续费不得为负,净到账必须大于0;实际金额遵守USD精度及通道限额。

例如申请USD 100.00、手续费USD 2.00,钱包及VIP金额占用均为USD 100.00,预计到账USD 98.00。不能在这笔报价之外再扣USD 2.00,也不能只占用98而申请100。费用和通道配置由原所属能力提供,不在Account新增费率开关。

报价缺项、过期或通道不能提供上述口径时不受理;零费用必须是明确报价结果。提交时费用或到账条件变化,先返回最新报价让玩家重新确认,不静默加扣或减少净到账。受理后固化本单金额,后续配置变化不重算;该项仅定义本期USD玩家渠道接入要求,不改变其他币种订单。玩家本人授权按账户会话核验,不使用本页后台审核权限码。
...
玩家申请被受理前,按 M4-D65 同时校验当前 VIP 等级的当日最高次数与标准币金额,原子占用额度并把判定快照写入订单;本页审核不因之后升降级或改配置而推翻已受理结果。

配置或来源：规则在 M4.6 配,M5 执行；****。
...
对一笔待审核订单点击通过或拒绝,填写必要信息并确认。

输入信息：审核结果(通过 / 拒绝)· 拒绝原因(拒绝时**必填**)· 备注。

1. 判权限;校验该单当前由本人锁定(见 R4),否则拒绝操作
2. **提交时二次校验提现资格**:该币种是否仍存在未完成的打码任务。
   打开页面时可提、提交时已不可提的,**按提交时为准拒绝**,不按打开时的状态放行
3. VIP 每日出款限额已在申请受理时按 A8 原子校验并固化;审核时不按玩家当前等级或最新配置重算,避免已受理订单被追溯推翻
4. 通过:订单转入放款流程,资金与该申请日的 VIP 次数 / 金额额度都保持占用,直至放款结果返回
5. 拒绝:必须填写原因;**解除本单的资金占用**,金额回到玩家可用余额;资金归位成功后同步释放该申请日的 VIP 次数与金额占用
6. 拒绝**不改变玩家的打码义务** —— 提现被拒不等于打码作废,也不等于额外惩罚
7. 写账变流水与审核审计,记录审核人、时间、结果与原因

### 批量审核(5.1-R3)

勾选多笔待审核订单后点击批量审核,选择结果并确认。

输入信息：勾选的订单集合(不超过 A4)· 批量结果(通过 / 拒绝)· 原因(拒绝时必填)。
...
打开某笔订单的审核操作 · 离开审核 · 锁定超时。

输入信息：无(由打开与关闭审核动作隐式触发)。
...
明确失败且未付款时解除原资金占用,打码义务不变,同时释放原申请日VIP额度。结果未知保持占用并查询原订单,禁止重放款或提前退回。
...
| E6 | 放款回执丢失 | 通道已放款但平台未收到回执 | 订单保持放款中,不自行判定;资金与 VIP 出款额度继续占用,由周期对账捕获并告警 |
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 5.1-R1 | 订单列表与筛选：进入页面 · 点查询 · 翻页 | 12 项筛选可组合;默认筛选见 A2;敏感字段脱敏;汇总准确 |
| 5.1-R2 | 单笔审核：对某笔待审核订单点通过或拒绝 | 提交时二次校验提现资格;通过后进入放款;拒绝须填原因并解除资金占用 |
| 5.1-R3 | 批量审核：勾选多笔后点批量审核 | 逐笔独立判定,部分失败不影响其余;结果逐笔可见;单次上限见 A4 |
| 5.1-R4 | 锁定与并发控制：打开某笔订单的审核 | 同一笔单同时只能被一人占用;超时自动释放见 A7 |
| 5.1-R5 | 自动刷新：点开启自动刷新 | 按 A3 间隔刷新列表;刷新不打断正在进行的审核操作 |
| 5.1-R6 | 全量导出：点全量导出数据 | 不阻塞页面,完成后通知;内容与当前筛选一致;敏感字段按权限脱敏 |
| 5.1-R7 | USD申请100、内扣费用2并成功受理 | 钱包及VIP金额占用100,预计到账98,报价与订单依据一致,不额外扣2 |
| 5.1-R7 | 报价缺项、过期、费用为负、净到账不大于0或提交时费用变化 | 非法或缺失不受理;变化须重新确认,不默认零费用或沿旧净额扣款 |
...
- 边界:申请后玩家降级或运营调低出款上限 → 已受理订单仍可继续审核,本页展示申请时快照;若拒绝则在资金归位后释放原申请日额度
...
- 跳出:玩家详情弹窗(点用户ID)· 5.3 人工充值(错账回收场景)· 5.8 审核规则(调整分流条件)
- 用户详情的提款记录只查询当前用户的提现扣款账变,不提供跳往本页的按钮。

### 权限

| 操作 | 权限或执行方 |
|---|---|
| 订单列表与筛选(5.1-R1) | `finance:withdraw:order` |
| 单笔审核(5.1-R2) | `finance:withdraw:order:review` |
| 批量审核(5.1-R3) | `finance:withdraw:order:review` |
| 锁定与并发控制(5.1-R4) | `finance:withdraw:order:review` |
| 自动刷新(5.1-R5) | `finance:withdraw:order` |
| 全量导出(5.1-R6) | `finance:withdraw:order:export` |
...
拆分语义统一引用资金模型。用户详情的提款记录展示提现扣款账变,字段按流水定义,不复制本页的订单列。

### 依赖与数据流

**数据流向**:玩家在客户端发起提现 → 平台占用对应金额并生成订单,经 5.8 自动审核分流
(按标签处置与总开关决定直接放款或转待审核,判定快照随单)→
财务在本页锁定并审核 → 通过则提交放款、拒绝则解除占用 →
通道执行放款并回执 → 平台按回执更新订单状态并写流水 → 订单处理结果呈现在本页,资金变化可在用户详情的资产变动中查询。
...
- 拥有(owner=M5):提现订单 · 账变流水 · 审核审计 · 锁定状态 · VIP 每日出款额度占用 / 释放记录与申请时判定快照
- 只读引用(owner=M4):玩家当前 VIP 等级与逐级每日出款次数 / 金额上限,仅用于申请受理前判定(D65)
...
| 归属 | 做什么 |
|---|---|
| 界面 | 展示该玩家的余额构成、打码进度、风控标签与 5.8 判定快照(命中条件与当时取值);拒绝原因必填;防重复提交 |
