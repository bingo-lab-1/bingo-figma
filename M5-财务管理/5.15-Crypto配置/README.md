...
让授权人员在后台安全配置 Crypto 入金支持的链、币种和 RPC 节点,
...
在不发版的情况下扩展多链多币种,同时通过验证、双人审批、版本快照和节点冗余
降低错账、漏账、资损与服务中断风险。

### 本次范围

- 不展示、导入或管理助记词、根密钥、私钥和派生私钥
- 不在本页执行链上归集、提现、Gas 补充或交易签名；归集控制面与执行边界见
  `../5.14-Crypto入金/Gas-Station与自动归集方案.md`
...
| 配置或参数 | 填写与执行规则 |
|---|---|
| RPC健康检查周期(A1) | 30 秒；系统级 Crypto 配置 |
| 单次 RPC 超时(A2) | 10 秒；每个 RPC 节点,可继承系统默认 |
| 节点落后阈值(A3) | 按链配置,默认 5 个区块；链配置；不同链出块速度不同 |
| 自动摘除阈值(A4) | 连续 3 次健康检查失败；系统级 Crypto 配置 |
| 启用网络的节点下限(A5) | 至少 2 个来自独立供应源的健康节点；链配置；规避单节点故障 |
| Token精度(A8) | 首次配置时必须与链上元数据一致;产生入金后不可直接修改；币种配置；防止数量级错账 |
| RPC密钥展示(A9) | 保存后永不回显明文,只展示脱敏结果；凭据安全要求 |
| 测试网隔离(A11) | 测试网与主网配置、地址和交易完全隔离;生产默认不展示测试网；防止测试资产混入生产 |
...
提交人与审批人必须是不同账号,一人不得自批。

配置或来源：资金安全四眼原则。

### 配置生效方式(A7)

审批通过后生成新版本并立即生效;历史处理不回溯。

配置或来源：保证规则可追溯。

### 紧急停用时效(A10)

授权操作确认后 1 分钟内阻止新入金自动入账。
...
持有链编辑权限的账号新增或编辑链配置并提交。

输入信息：“配置项”链配置表单字段;网络代码、环境及已经产生入金的网络身份不可修改。
...
持有币种编辑权限的账号新增或编辑币种配置并提交。

输入信息：“配置项”币种配置字段;关联项保存 Currency ID,
...
持有 RPC 编辑权限的账号新增、编辑或更换节点地址及凭据。

输入信息：“配置项” RPC 表单字段;地址与凭据只允许写入,保存后不可读取明文。
...
### RPC健康检查与自动切换(5.15-R5)

按 A1 定时检查、生产调用失败、节点限流、节点恢复或同链高度出现分歧。

输入信息：节点响应、网络身份、最新高度、延迟、错误分类与同链其他节点的对照结果。
...
编辑人提交配置;审批人查看差异并批准或拒绝。

输入信息：变更对象 · 变更前后值 · 提交理由 · 审批意见;敏感值只显示“已新增/已替换”。
...
持有紧急停用权限的账号对链或币种执行紧急停用并二次确认。

输入信息：停用对象 · 原因(必填) · 事件/工单号(可选)。
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 5.15-R1 | 查看配置与版本：进入页面 · 切换 tab · 查询 | 链、币种、RPC、版本和健康状态可追溯,敏感字段脱敏 |
| 5.15-R2 | 新增编辑链配置：点击新增/编辑并提交 | 网络身份、确认、地址和监听规则完整校验,不直接覆盖生效版本 |
| 5.15-R3 | 新增编辑币种配置：点击新增/编辑并提交 | 链上资产唯一关联一个可充值 Currency;精度经链上验证,同名假币不能映射为正版币种 |
| 5.15-R4 | 新增编辑RPC节点：点击新增/编辑并测试 | 网络身份、用途与连接通过验证,凭据保存后不回显 |
| 5.15-R5 | RPC健康检查与自动切换：定时检查 · 调用失败 · 节点恢复 | 异常节点按 A4 摘除,健康节点承接任务,恢复后经检查再加入 |
| 5.15-R6 | 提交与审批配置：提交审批 · 审批通过/拒绝 | 按 A6 双人复核,通过后生成 A7 版本,拒绝不影响当前版本 |
| 5.15-R7 | 启用链或币种：对待启用配置提交并审批 | 节点、资产、风控和地址规则全部就绪后才对玩家开放 |
| 5.15-R8 | 紧急停用：点击紧急停用并二次确认 | A10 内阻止新入金自动入账,历史事实不删除,操作完整留痕 |
| 5.15-R9 | 手动测试RPC：点击测试连接 | 返回网络身份、最新高度、延迟与能力结果,不泄露凭据 |
...
| 操作 | 权限或执行方 |
|---|---|
| 查看配置与版本(5.15-R1) | `finance:recharge:crypto-config` |
| 新增编辑链配置(5.15-R2) | `finance:recharge:crypto-config:edit-chain` |
| 新增编辑币种配置(5.15-R3) | `finance:recharge:crypto-config:edit-asset` |
| 新增编辑RPC节点(5.15-R4) | `finance:recharge:crypto-config:edit-rpc` |
| RPC健康检查与自动切换(5.15-R5) | `finance:recharge:crypto-config` |
| 提交与审批配置(5.15-R6) | `finance:recharge:crypto-config:approve` |
| 启用链或币种(5.15-R7) | `finance:recharge:crypto-config:enable` |
| 紧急停用(5.15-R8) | `finance:recharge:crypto-config:disable` |
| 手动测试RPC(5.15-R9) | `finance:recharge:crypto-config:test-rpc` |
