...
让运营在不经支付通道的情况下,安全地给玩家加钱或扣钱,且每一笔都能被追溯与复核。
...
把「改数据库」这种没有痕迹的操作,变成有权限、有原因、有流水、可复核的正规动作。
这是资金安全的底线,也是出事后唯一能自证清白的东西。

### 本次范围
- 不做打码义务的直接干预(新增打码量、逐条清除在用户详情的打码任务页签内完成,本页不重复提供入口)
- 不做审批流 —— 本版采用即时生效与事后复核
- 单笔操作只作用于一个玩家的一个币种;批量上传可跨玩家跨币种,但仍是逐行独立处理
- 不定义资金语义本身(余额、真金彩金、打码任务的含义见“相关资料”引用的资金模型)
...
**有意不做的字段**:不提供跨币种一次调整多个钱包的入口,理由见“用途”。**不要补上。**

### 配置限制与默认值

| 配置或参数 | 填写与执行规则 |
|---|---|
| 单笔金额上限(A1) | 10000；配置项,**界面须显式展示当前上限** |
| 打码倍数上限(A2) | 10000；配置项 |
| 扣款是否受 A1 约束(A3) | 受同一上限；同 A1 |
| 备注长度上限(A4) | 200 字符 |
| 批量充值单次行数上限(A6) | 500 行；配置项 |
| 批量文件格式(A7) | `.xlsx` `.xls` `.csv` |
| 操作流水号格式(A8) | 32 位以内,字母数字与短横,全表唯一 |
...
方向选「充值」,填写用户ID、币种、至少一个金额与其对应倍数,点击确定并通过二次确认。

输入信息：用户ID(数字)· 币种(启用中)· 真金金额与真金流水倍数 · 彩金金额与彩金流水倍数 ·
...
方向选「扣款」,填写用户ID、币种与金额,点击确定并通过二次确认。

输入信息：用户ID · 币种 · 真金金额 · 彩金金额 · 备注(必填)。**倍数框不可填**。
...
充值方向下,真金流水倍数或彩金流水倍数任一填 0 并提交。

输入信息：同 R1,其中至少一个倍数为 0;备注必填。
...
5. 两个倍数框的标签必须**分别标明所属轨**(真金流水倍数 / 彩金流水倍数),
   不得两个都叫「流水倍数」;金额框标签不得带方向含义。理由见“查询与处理信息”

### 批量充值(5.3-R5)

切到批量充值 tab,下载模板,填好后上传文件并点击确定上传。

输入信息：数据文件(格式见 A7,行数上限见 A6)。每行的字段与单个充值一致,
...
| 批量执行预览 | 解析成功后先展示汇总(行数、两个方向各自的笔数与金额、涉及币种),确认后才执行 |
...
| 需求编号 | 场景与操作 | 预期结果 |
|---|---|---|
| 5.3-R1 | 人工充值(加钱)：方向选「充值」并提交 | 两轨金额各自独立入账并各自建打码任务;倍数缺失整笔拒绝;余额与流水同增 |
| 5.3-R2 | 人工扣款：方向选「扣款」并提交 | 不建打码任务;不减免已有打码义务;扣至负数直接拒绝 |
| 5.3-R3 | 免打码入账：充值方向且任一倍数填 0 | 需同时持有充值与免打码两个节点;照常建需打码量为 0 的任务并留痕 |
| 5.3-R4 | 表单联动与上限提示：打开页面 · 切换方向 · 输入超限 | 界面显式展示当前上限;扣款方向下倍数框不可填;两个倍数框标签可区分 |
| 5.3-R5 | 批量充值：下载模板 · 上传文件 · 确定上传 | **逐行按方向校验对应权限**;逐行独立入账,部分失败不影响其余;需幂等键 |
...
- 菜单:财务管理 → 充值管理 → 人工充值
- 跳入:充值订单查询页对掉单玩家的补偿跳转

- 跳出:5.4 人工充值操作记录(按本次用户ID 过滤)· 玩家详情弹窗的资产变动
- 用户详情的钱包在弹窗内直接提供修改余额,不跳往本页;两个入口的操作说明分别维护,共用资金语义仍见资金模型。

### 权限

| 操作 | 权限或执行方 |
|---|---|
| 人工充值(加钱)(5.3-R1) | `finance:recharge:manual` |
| 人工扣款(5.3-R2) | `finance:recharge:manual:deduct` |
| 免打码入账(5.3-R3) | `finance:recharge:manual` + `finance:recharge:manual:waive` |
| 表单联动与上限提示(5.3-R4) | `finance:recharge:manual` |
| 批量充值(5.3-R5) | `finance:recharge:manual:batch` |
...
界面标签按上方“查询与处理信息”的硬要求改为「真金金额 / 彩金金额」,
