...
| 8.1-R4 | 保存与留痕 | 二次确认显示变更 diff;写 操作日志 | `system:config:edit` | 0.5 |

## 5. 状态清单
加载 / 正常 / 保存中 / 保存成功 / 校验错误 / 无权限(只读)

## 6. 交互流程
保存即生效( 是否需发布流程待定),变更强制写 操作日志。

## 7. 涉及表
键值配置(owner,建议补分组/类型/描述字段)
